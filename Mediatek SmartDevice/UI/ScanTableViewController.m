...
- (void)viewDidLoad
{
    ...
    NSLog(@"version = %@, %.02f", [[UIDevice currentDevice] systemVersion], iOSVersion);
    //request authorization for healthkit data
    if (iOSVersion >= 8.0f) {
        [self performSelector: @selector(requestHealkitAuthoriztion) withObject: nil afterDelay: 1];
    }
...
- (void)requestHealkitAuthoriztion {
    NSLog(@"[ScanTableViewController] [requestHealkitAuthoriztion] enter");
